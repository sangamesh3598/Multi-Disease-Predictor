...
# load malaria model path
MALARIA_MODEL_PATH = 'src/model/Malaria-Detection/malaria_model.h5'
...
def malaria():
    if request.method == 'POST':
        try:
            ...
            f.save(file_path)
            # Make prediction
            preds = malaria_predict(file_path, model)
            os.remove(file_path)#removes file from the server after prediction has been returned
            if preds >= 0.5:
                res = "Sorry :( you have got the chances of Malaria"
            else:
                res = "Congratulations! you are safe from Malaria"
            return render_template('malaria.html',prediction=res)
        except Exception as e:
            print(e)
